...
import { CheckCircle, Zap, Shield, Clock, AlertCircle } from "lucide-react";
...
export default function AccountsDialog({
  ...
  const [projectsCountError, setProjectsCountError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/projects/count")
      ...
      .then((data) => {
        if (!isCancelled) {
          if (data.error) setProjectsCountError(data.error);
          else setProjectsCount(data.count);
        }
      })
      .catch((err) => {
        if (!isCancelled) setProjectsCountError(err.message);
      })
      .finally(() => {
    ...
    fetch("/api/payment/history")
      .then((r) => r.json())
      .then((data) => {
        if (!isCancelled && data.history) setPassHistory(data.history);
      })
      .catch(() => {})
      .finally(() => {
  ...
  const accountTitle = !isLoaded ? "Loading..." : user ? displayName : "Not signed in";
  const accountSubtitle = !isLoaded
    ? "Checking session…"
    : user?.primaryEmailAddress?.emailAddress
      ? user.primaryEmailAddress.emailAddress
      : user
