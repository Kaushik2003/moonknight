...
const FRONTEND_MODELS: { id: AIModel; name: string; icon: string }[] = [
  { id: "groq:moonshotai/kimi-k2-instruct-0905", name: "Kimi K2", icon: "/kimi.png" },
  { id: "gpt-4o-mini", name: "GPT-4o-mini", icon: "/openai.jpg" },
  { id: "gemini-2.5-flash", name: "Gemini 2.5 Flash", icon: "/gemini.webp" },
...
export function ChatPanel({
  ...
  // Sandbox context for frontend mode
  const sandbox = useSandboxContext();
  const isFrontendMode = chatContext.resolvedMode === 'frontend';
  const isSandboxReady = isFrontendMode && sandbox.isConnected;

  // State for sandbox-routed chat (frontend mode)
  ...
  const { messages: contextMessages, addMessage: addContextMessage, setMessages: setContextMessages, pendingPrompt, setPendingPrompt, chatSessionId } = useChatContext();
          ...
          setMessages((prev) => [
            ...prev,
            {
              ...
              content: JSON.stringify({
                chat: {
                  message: '⚡ **Builder Pass required.** AI features are gated behind a Builder Pass (₹150 / 24h). Click the **Get Builder Pass** button in the top bar to unlock full access.',
                },
                editor: { changes: [] },
              }),
              createdAt: new Date(),
  ...
  // --- Sandbox chat handlers for frontend mode ---
  useEffect(() => {
    if (!isFrontendMode) return;

    // Register chat response handler
    sandbox.setOnChatResponse((response) => {
      console.log('[ChatPanel] Sandbox chat response received:', response.chat?.message?.substring(0, 100));
      ...
      const assistantMsg: ChatMessage = {
        ...
        role: 'assistant',
        content: response.chat?.message || 'No response received.',
        timestamp: new Date(),
        model: selectedModel,
      };
      setSandboxMessages(prev => [...prev, assistantMsg]);

      // Apply editor changes if any
      if (response.editor?.changes && response.editor.changes.length > 0 && onApplyEditorChanges) {
        console.log(`[ChatPanel] Applying ${response.editor.changes.length} editor changes from sandbox`);
        onApplyEditorChanges(response.editor.changes);
      }
    });

    // Register chat progress handler
    sandbox.setOnChatProgress((progress) => {
      const phaseLabels: Record<string, string> = {
        'intent_resolved': '🔍 Analyzing intent...',
        'planning': '📝 Planning...',
        'execution_complete': '⚙️ Executing...',
        'validation_passed': '✅ Validating...',
        'validation_failed': '⚠️ Retrying...',
      };
      setSandboxProgress(phaseLabels[progress.phase] || `Working: ${progress.phase}`);
    });

    return () => {
      sandbox.setOnChatResponse(null);
      sandbox.setOnChatProgress(null);
    };
  }, [isFrontendMode, sandbox, selectedModel, onApplyEditorChanges]);

  // Sandbox chat send function for frontend mode
  const sendViaSandbox = useCallback((prompt: string) => {
    if (!sandbox.isConnected) return;

    // Add user message to sandbox messages
    ...
    setSandboxProgress('🔍 Analyzing request...');

    // Send via sandbox WebSocket
    sandbox.sendChatMessage(prompt, {
      activeFilePath: chatContext.activeFilePath,
      fileContents: fileContents || {},
      mode: chatContext.resolvedMode,
      model: selectedModel,
  ...
  }, [sandbox, selectedModel, chatContext.activeFilePath, chatContext.resolvedMode, fileContents]);
  ...
  useEffect(() => {
    ...
    if ((intent === 'explain' || intent === 'debug') && selection?.selectedText) {
      ...
      // Premium Model Gatekeeping Check
      const isPremiumModel = ['gpt-4o-mini', 'claude-sonnet'].includes(selectedModel);
      if (isPremiumModel && !isPassActive) {
        setShowPricingModal(true);
        // Clear tracking so they can try again after purchasing
  ...
  const contractMessages: ChatMessage[] = aiMessages
    .filter(msg => {
  ...
  // In frontend mode, use sandbox messages; in contract mode, use useChat messages + test results
  const messages: ChatMessage[] = isFrontendMode ? sandboxMessages : [...contractMessages, ...testMessages];
  ...
  // Determine effective loading state — prefer agentState.isLoading when in frontend mode
  const effectiveLoading = isFrontendMode ? (sandboxLoading || sandbox.agentStatus?.isLoading) : isLoading;
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    // Frontend mode: route through sandbox WebSocket
    if (isFrontendMode) {
      if (!sandbox.isConnected) {
        // Block send — sandbox not connected
        setSandboxMessages(prev => [...prev, {
          id: `system-${Date.now()}`,
          role: 'assistant',
          content: '⚠️ **Sandbox not connected.** Please start the sandbox first to chat in frontend mode.',
          timestamp: new Date(),
        }]);
        return;
      }
      ...
      // Send via sandbox
      sendViaSandbox(prompt);
      return;
    }

    // Smart contract mode: use existing useChat flow
    // Call parent handler
  ...
  useEffect(() => {
    if (pendingPrompt && !isLoading) {
      ...
      if (isPremiumModel && !isPassActive) {
        setShowPricingModal(true);
        // Put the prompt back so it can be sent after purchase
      ...
      <div className="flex h-full">
        ...
        {isChatOpen && (
          ...
          >
            ...
            {/* Agent Status Badge — shown in frontend mode */}
            {isFrontendMode && (
              <div className="px-4 py-2 border-b border-zinc-800/50">
                <AgentStatusBadge
                  status={sandbox.agentState.status}
                  currentTool={sandbox.agentState.currentTool}
                  currentFile={sandbox.agentState.currentFile}
                  retryCount={sandbox.agentState.retryCount}
                  maxRetries={sandbox.agentState.maxRetries}
                  retryReason={sandbox.agentState.retryReason}
                  tokenUsage={sandbox.agentState.tokenUsage}
                  lastError={sandbox.agentState.lastError}
                />
              </div>
            )}
                              ...
                              if (model) {
                                return (
                                  <img
                                    src={model.icon}
                                    alt={model.name + " icon"}
                                    style={{ width: 16, height: 16, borderRadius: 4, objectFit: "cover", display: "inline-block", verticalAlign: "middle" }}
                                  />
                                );
                              }
                              return null;
                ...
                >
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-sm text-zinc-300 bg-zinc-900/50 p-3 rounded-r-lg rounded-bl-lg border border-zinc-800/50 flex items-center gap-2">
                      ...
                      <span className="text-zinc-500">{sandboxProgress || 'Thinking...'}</span>
            ...
            <div className="p-3 bg-zinc-900/30">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                        ...
                        if (model) {
                          return (
                            <img
                              src={model.icon}
                              alt={model.name + " icon"}
                              style={{ width: 18, height: 18, borderRadius: 4, objectFit: "cover", display: "inline-block", verticalAlign: "middle" }}
                            />
                          );
                        }
                        return null;
                    ...
                    <AnimatePresence>
                      {isModelDropdownOpen && (
                        ...
                        >
                          {(chatContext.resolvedMode === 'frontend' ? FRONTEND_MODELS : MODELS).map((model) => (
                            <button
                              key={model.id}
                              onClick={() => {
                  ...
                  {/* Test Button - Contract mode only */}
                  {chatContext.resolvedMode === 'contract' && (
                    <button
                      onClick={async () => {
                        if (isTesting || !chatContext.activeFilePath) return;
                        const code = fileContents?.[chatContext.activeFilePath] || '';
                        if (!code.trim()) return;

                        setIsTesting(true);
                        // Add a user message showing the test was triggered
                        const testMsg: ChatMessage = {
                          id: `test-req-${Date.now()}`,
                          role: 'user',
                          content: `🧪 Testing contract: ${chatContext.activeFilePath.split('/').pop()}`,
                          timestamp: new Date(),
                          model: selectedModel,
                        };
                        ...
                        try {
                          const res = await fetch('/api/chat/test', {
                            ...
                            body: JSON.stringify({
                              code,
                              fileName: chatContext.activeFilePath,
                              model: selectedModel,
                              fileContents,
                              chatSessionId,
                            }),
                          ...
                          setTestMessages(prev => [...prev, resultMsg]);
                        } catch (err) {
                          const errMsg: ChatMessage = {
                            id: `test-err-${Date.now()}`,
                            role: 'assistant',
                            content: `❌ Test failed: ${err instanceof Error ? err.message : 'Unknown error'}`,
                            timestamp: new Date(),
                            model: selectedModel,
                          };
                      ...
                      disabled={isTesting || !chatContext.activeFilePath}
                      className={cn(
                        "flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-all duration-200 border",
                        isTesting
