...
import { useAgentState, isAgentSignal } from "./useAgentState";
import type { AgentSignal } from "./useAgentState";
...
export interface UseSandboxReturn {
  ...
  // Agent state machine (from useAgentState)
  ...
  setOnChatResponse: (
    handler: ((response: ChatResponse) => void) | null,
  ) => void;
  onChatProgress: ((progress: ChatProgress) => void) | null;
  setOnChatProgress: (
    handler: ((progress: ChatProgress) => void) | null,
  ) => void;
  onBindingsResponse: ((response: BindingsResponse) => void) | null;
  setOnBindingsResponse: (
    handler: ((response: BindingsResponse) => void) | null,
  ) => void;
  ...
  setOnFileTreeSync: (
    handler: ((data: FileTreeSyncData) => void) | null,
  ) => void;
  onFileContentSync: ((data: FileContentSyncData) => void) | null;
  setOnFileContentSync: (
    handler: ((data: FileContentSyncData) => void) | null,
  ) => void;
...
export function useSandbox(): UseSandboxReturn {
  ...
  // Agent state machine — processes AgentSignal frames
  const agentState = useAgentState();
  // Store stable refs to avoid stale closures in socket handlers
  ...
  const onChatResponseRef = useRef<((response: ChatResponse) => void) | null>(
    null,
  );
  const onChatProgressRef = useRef<((progress: ChatProgress) => void) | null>(
    null,
  );
  const onBindingsResponseRef = useRef<
    ((response: BindingsResponse) => void) | null
  >(null);
  const onFileResponseRef = useRef<((data: any) => void) | null>(null);
  const onFileTreeSyncRef = useRef<((data: FileTreeSyncData) => void) | null>(
    null,
  );
  const onFileContentSyncRef = useRef<
    ((data: FileContentSyncData) => void) | null
  >(null);
  ...
  const initSocket = useCallback(() => {
    ...
    const socket = io(API_URL, {
      ...
      reconnectionDelay: 2000,     // min 2s between reconnect attempts
    ...
    socket.on("disconnect", (reason) => {
      ...
      if (reason !== "io client disconnect") {
        ...
        // Set agent state to error with recoverable flag
        agentStateRef.current.setDisconnected("Connection lost \u2014 reconnecting\u2026");
      }
    });

    // On successful reconnect, reset agent state and re-request health
    socket.io.on("reconnect", () => {
      addLog("Reconnected to backend");
      agentStateRef.current.reset();
      // Re-request sandbox health
      if (sessionIdRef.current) {
        socket.emit("sandbox:status", { sessionId: sessionIdRef.current });
      }
    });

    // Sandbox events
    socket.on("sandbox:spawned", (data) => {
      addLog(`Sandbox container started — waiting for preview to be ready…`);
      setSandboxInfo({
      ...
      // Container + agent are up, but Next.js dev server is still booting.
      // Stay in "preview_starting" until sandbox:preview_ready fires.
      setStatus("preview_starting");
    });

    socket.on("sandbox:preview_ready", (data) => {
      addLog(`Preview ready at ${data.previewUrl}`);
      // Update previewUrl in case it changed, then mark fully running.
      setSandboxInfo((prev) =>
        prev ? { ...prev, previewUrl: data.previewUrl } : prev,
      );
      setStatus("running");
    });
    ...
    socket.on("sandbox:message", (message) => {
      // Process AgentSignal frames through the state machine
      if (isAgentSignal(message)) {
        agentStateRef.current.processSignal(message as AgentSignal);
      }
      ...
      switch (message.type) {
        ...
        case "bindings_generated":
          ...
          console.log(
            "[useSandbox] Changes count:",
            message.editor?.changes?.length || 0,
          );
          console.log("[useSandbox] Error:", message.error || "none");
          console.log(
            "[useSandbox] Chat message:",
            message.chat?.message?.substring(0, 100) || "none",
          );
          ...
          if (message.success) {
            ...
            if (message.editor?.changes) {
              const bindingsFiles = message.editor.changes.filter(
                (c: EditorChange) => c.path.startsWith("bindings/"),
              );
              const aiFiles = message.editor.changes.filter(
                (c: EditorChange) => !c.path.startsWith("bindings/"),
              );
              console.log("[useSandbox] Bindings files:", bindingsFiles.length);
              console.log("[useSandbox] AI-generated files:", aiFiles.length);
              addLog(
                `[bindings] ${bindingsFiles.length} bindings files, ${aiFiles.length} AI integration files`,
              );
            }
          ...
          if (onBindingsResponseRef.current) {
            ...
            console.warn(
              "[useSandbox] No onBindingsResponse handler registered",
            );
          }
          console.log(
            "[useSandbox] ============================================",
  ...
  const generateBindings = useCallback(
    (contractId: string, network: "testnet" | "mainnet") => {
      ...
      console.log(
        "[useSandbox] Emitting sandbox:generate_bindings with payload:",
        payload,
      );
  ...
  return {
    ...
    setOnChatResponse: (handler) => {
      onChatResponseRef.current = handler;
    },
    onChatProgress: onChatProgressRef.current,
    setOnChatProgress: (handler) => {
      onChatProgressRef.current = handler;
    },
    onBindingsResponse: onBindingsResponseRef.current,
    setOnBindingsResponse: (handler) => {
      onBindingsResponseRef.current = handler;
    },
    onFileResponse: onFileResponseRef.current,
    setOnFileResponse: (handler) => {
      onFileResponseRef.current = handler;
    },
    onFileTreeSync: onFileTreeSyncRef.current,
    setOnFileTreeSync: (handler) => {
      onFileTreeSyncRef.current = handler;
    },
    onFileContentSync: onFileContentSyncRef.current,
    setOnFileContentSync: (handler) => {
      onFileContentSyncRef.current = handler;
    },
    clearLogs,
    reset,
